...
**② 키 넣기.** 서버에는 브라우저가 없으니 **내 PC 브라우저를 빌려 씁니다.**
서버에서 `python setup.py` 를 실행하면, 화면을 열었다고 하지 않고 아래 한 줄을
알려 줍니다(서버 주소까지 채워서). 그 줄을 내 PC 터미널에 붙여넣고 창을 켜 두면
`http://127.0.0.1:8777` 이 내 브라우저에서 열립니다.

```bash
python setup.py                                 # 서버에서
ssh -N -L 8777:127.0.0.1:8777 root@서버주소     # 내 PC에서 (창은 켜 둡니다)
```

**포트를 열 필요도, 방화벽을 만질 필요도 없습니다.** 키는 SSH 안으로만 지나갑니다.
설정 화면은 `.env`를 고치고 파이썬을 실행하는 화면이라, 인터넷에 그냥 내놓으면 안 됩니다.

내 PC에서 SSH를 쓸 수 없다면(클라우드 웹 콘솔뿐이라면) 그때만:
