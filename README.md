...
4. 내가 "연결됐다"고 하면, 질문을 통해서 내 투자 성향을 파악하고 strategy.py를 새롭게 만들어줘.
...
"판단해줘"가 비어 있으면 아무것도 하지 말고 그대로 끝내.
...
여기까지가 **로컬**입니다. 며칠 돌려 보고 잘되면 클라우드로 옮겨 컴퓨터를 꺼도
됩니다 → [클라우드로 옮기기](#클라우드로-옮기기-컴퓨터를-꺼도-됩니다)
...
## 클라우드로 옮기기 (컴퓨터를 꺼도 됩니다)

**먼저 로컬 예약으로 며칠 돌려 보세요.** 판단이 멀쩡한지, 주문이 제대로 나가는지
눈으로 확인한 뒤에 옮기면 됩니다. **코드는 그대로입니다** — 예약의 실행 위치만
`이 기기` 에서 `클라우드` 로 바뀝니다.
([공식 문서](https://learn.chatgpt.com/docs/cloud))

**① 로컬 예약을 일시 중지합니다.** 이걸 먼저 하세요. 둘 다 켜 두면 같은 계좌에
두 번 주문합니다. 다시 로컬로 돌아올 때는 반대로 하면 됩니다.

**② 저장소를 GitHub에 올립니다.** 클라우드는 GitHub에서 코드를 받아 갑니다.
`.env`는 `.gitignore`에 있어 올라가지 않습니다 — **올리지 마세요.** 키는 ③에서 넣습니다.
상담으로 `strategy.py`를 고칠 때마다 커밋·푸시해야 클라우드가 그 전략을 씁니다.

**③ Environment를 만듭니다.** Codex → 설정 → Environments → 저장소 선택.

일반 값은 **환경 변수**에, 키는 **비밀(Secrets)** 에 넣습니다.
로컬 `.env`에 넣은 값과 같은 것을 그대로 옮기면 됩니다.

| 환경 변수 | 값 |
|---|---|
| `NH_MOCK` | **`1` = 모의투자 · `0` = 실제 계좌** |
| `MASK_MONEY` | `0` (촬영·화면공유 중이면 `1`) |

| 비밀 (Secrets) | 값 | 실거래에 필요? |
|---|---|---|
| `NHPLUG_APP_KEY` | NH PLUG 앱키 | 필수 |
| `NHPLUG_APP_SECRET` | NH PLUG 시크릿 | 필수 |
| `TELEGRAM_BOT_TOKEN` | 봇 토큰 | **필수** (모의투자면 생략 가능) |
| `TELEGRAM_CHAT_ID` | 채팅 ID | **필수** (모의투자면 생략 가능) |

`.env` 파일이 없으면 코드가 환경 변수를 그대로 읽습니다. 고칠 것은 없습니다.

실거래로 바꿀 때는 `NH_MOCK`을 `0`으로 고치고 **Telegram 두 값을 반드시 넣으세요.**
`NH_MOCK=0`인데 Telegram이 없으면 주문 자체를 하지 않습니다 — 승인 없이 실제 돈이
나가는 길을 두지 않기 위해서입니다.

**④ 설정 스크립트**에 이 한 줄을 넣습니다. 설치 단계는 인터넷이 열려 있습니다.

```bash
pip install -r requirements.txt
```

**⑤ 에이전트 인터넷 연결을 켜고** 아래 네 개를 허용 도메인에 추가합니다.

```
api.nhplug.com      인증 (모의투자여도 필요)
moapi.nhplug.com    모의투자 주문·조회
api.telegram.org    실거래 승인
news.google.com     뉴스 제목
```

**허용된 HTTP 메서드는 `모든 메서드`** 로 둡니다. 주문과 토큰 발급이 POST라
GET만 열면 조회는 되고 주문이 안 됩니다.

**⑥ 예약을 새로 만듭니다.** 실행 위치만 **클라우드**로 고르고, 제목·간격·내용은
로컬 예약과 똑같이 넣습니다.

### 알아 둘 것

- **모의투자로 시작하세요.** 인터넷을 켠 환경에서 뉴스를 읽고 키를 쥐고 있게 됩니다.
  공식 문서도 인터넷을 켜면 프롬프트 주입·비밀 유출 위험이 있다고 안내합니다.
- **`.cache/`가 매 실행마다 초기화됩니다.** 시세는 매번 새로 받으므로 문제는 없지만,
  "방금 시도한 주문" 기록이 사라져 같은 주문을 다시 시도할 수 있습니다.
- 로컬과 클라우드 중 **한쪽만 켜 두세요.** 둘 다 켜면 같은 계좌에 두 번 주문합니다.
...
`.env`에 `MASK_MONEY=1` 을 넣으면 잔고가 `***` 로 가려집니다.
주가는 공개 정보라 가리지 않습니다. 계좌번호는 언제나 `500***69` 처럼 나옵니다.
...
## 주의
- 로컬 예약은 컴퓨터가 꺼져 있으면 돌지 않습니다. 손절도 같이 멈춥니다.
  꺼 두려면 위 **"클라우드로 옮기기"** 를 보세요.
